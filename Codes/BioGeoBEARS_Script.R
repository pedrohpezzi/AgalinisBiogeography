...
library(GenSA)    # GenSA seems better than optimx (but slower) on 5+ parameters, seems to sometimes fail on simple problems (2-3 parameters)
library(rexpokit)
library(cladoRcpp)
...
setwd("C:/Users/pedro/OneDrive - University of Arkansas/Agalinis/Analyses/06_BioGeoBEARS")

# load data
trfn = "InputTrees/concatenated_tree_nosynonyms.nwk"
tr = read.tree(trfn)
tr
plot(tr, cex=0.5)
...
tipranges = getranges_from_LagrangePHYLIP(lgdata_fn=geofile)
tipranges
...
BioGeoBEARS_run_object$geogfn = geofile
...
runslow = TRUE
resfn = "AgalinisDEC_beast_concatenated.Rdata"
if (runslow)
{
...
resfn = "Agalinis_DEC+J_beast_concatenated.Rdata"
runslow = TRUE
if (runslow)
{
...
# PDF plots
#######################################################
pdffn = "Agalinis_DEC_vs_DEC+J_concatenated.pdf"
pdf(pdffn, height=6, width=6)

#######################################################
# Plot ancestral states - DEC
#######################################################
analysis_titletxt ="BioGeoBEARS DEC on Agalins - BEAST Concatenated"
...
# States
res2 = plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="text", label.offset=0.45, tipcex=0.7, statecex=0.7, splitcex=0.6, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges, plotlegend = TRUE)
...
#######################################################
analysis_titletxt ="BioGeoBEARS DEC+J on Agalinis - BEAST Concatenated"
...
# States
res1 = plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="text", label.offset=0.45, tipcex=0.7, statecex=0.7, splitcex=0.6, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges, plotlegend = TRUE)
...
#check legends here
# https://groups.google.com/g/biogeobears/c/zkHP_YpA9kY
...
runslow = TRUE
resfn = "Agalinis_DIVALIKE_beast_concatenated.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "Agalinis_DIVALIKE+J_beast_concatenated.Rdata"
runslow = TRUE
if (runslow)
{
  #sourceall("/Dropbox/_njm/__packages/BioGeoBEARS_setup/")
...
pdffn = "Agalinis_DIVALIKE_vs_DIVALIKE+J_beast_concatenated.pdf"
pdf(pdffn, height=6, width=6)

#######################################################
# Plot ancestral states - DIVALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE on Agalinis - BEAST Concatenated"
...
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE+J on Agalinis - BEAST Concatenated"
...
runslow = TRUE
resfn = "Agalinis_BAYAREALIKE_beast_concatenated.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "Agalinis_BAYAREALIKE+J_beast_concatenated.Rdata"
runslow = TRUE
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
...
pdffn = "Agalinis_BAYAREALIKE_vs_BAYAREALIKE+J_beast_concatenated.pdf"
pdf(pdffn, height=6, width=6)

#######################################################
# Plot ancestral states - BAYAREALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE on Agalinis - BEAST Concatenated"
...
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE+J on Agalinis - BEAST Concatenated"
...
# Loads to "restable"
save(restable, file="restable_agalinis_beast_concatenated.Rdata")
load(file="restable_agalinis_beast_concatenated.Rdata")

# Loads to "teststable"
save(teststable, file="teststable_agalinis_beast_concatenated.Rdata")
load(file="teststable_agalinis_beast_concatenated.Rdata")

# Also save to text files
write.table(restable, file="restable_agalinis_beast_concatenated.txt", quote=FALSE, sep="\t")
write.table(unlist_df(teststable), file="teststable_agalinis_beast_concatenated.txt", quote=FALSE, sep="\t")
...
# Also save to text files
write.table(restable_AIC_rellike, file="restable_AIC_rellike_beast_concatenated.txt", quote=FALSE, sep="\t")
write.table(restable_AICc_rellike, file="restable_AICc_rellike_beast_concatenated.txt", quote=FALSE, sep="\t")

# Save with nice conditional formatting
write.table(conditional_format_table(restable_AIC_rellike), file="restable_AIC_rellike_beast_concatenated_formatted.txt", quote=FALSE, sep="\t")
write.table(conditional_format_table(restable_AICc_rellike), file="restable_AICc_rellike_beast_concatenated_formatted.txt", quote=FALSE, sep="\t")
...
write.table(restable, file = "results_lnl__beast_concatenated.txt", row.names = FALSE)
fwrite(teststable, file ="results_AIC_beast_concatenated.csv")
...
write.table(range_probabilities, file="range_probabilities_DECj_concatenated.txt", quote=FALSE, sep="\t")

# Look at the file
moref("range_probabilities_DECj_concatenated.txt")
